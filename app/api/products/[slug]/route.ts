...
export async function GET(
  ...
  try {
    ...
    const variantes = await query(
      `SELECT v.*, (SELECT GROUP_CONCAT(valor SEPARATOR ',') FROM atributos_variante WHERE variante_id = v.id) as atributos
    ...
    const resenas = await query(
      `SELECT r.*, u.nombre as usuario_nombre, u.url_avatar
       ...
       WHERE r.producto_id = ? AND r.aprobada = 1 ORDER BY r.creado_en DESC`,
      [(producto as Record<string, unknown>).id as number]
    );
